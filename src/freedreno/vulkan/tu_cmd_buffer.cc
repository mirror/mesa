                  ...
                  const VkOffset2D *fdm_offsets)
{
   ...
   if (fdm) {
      for (unsigned i = 0; i < views; i++) {
         ...
         /* Offsets less than a bin size are accomplished by sliding the bins.
          * However once we shift a whole bin size then we reset the bins back
          * to where they were at the beginning and we need to adjust where
          * each bin is sampling from:
          *
          * x offset = 0:
          *
          * ------------------------------------
          ...
          * bins to the left, until we reach the whole bin size and reset the
          * bin positions.
          *
          * If we were forced to use a shared viewport, then we must not shift
          * over the bins and instead must shift when sampling because we
          * cannot shift the bins differently.
          */
         if (fdm_offsets) {
            offset = fdm_offsets[i];
         ...
         tu_fragment_density_map_sample(fdm,
                                        (x1 + MIN2(x2, fb->width)) / 2 + offset.x,
                                        (y1 + MIN2(y2, fb->height)) / 2 + offset.y,
   ...
   for (unsigned i = 0; i < views; i++) {
      ...
      while (tiling->tile0.width % tile->frag_areas[i].width != 0 ||
             (fdm_offsets && (fdm_offsets[i].x % tile->frag_areas[i].width != 0)))
         tile->frag_areas[i].width--;
      while (tiling->tile0.height % tile->frag_areas[i].height != 0 ||
             (fdm_offsets && (fdm_offsets[i].y % tile->frag_areas[i].height != 0)))
                     ...
                     const VkSubpassEndInfo *pSubpassEndInfo)
{
   ...
   const VkOffset2D *fdm_offsets =
      fdm_offset_info ? fdm_offset_info->pFragmentDensityOffsets : NULL;
