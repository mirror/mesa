...
#define inters_deref(b, deref, field) nir_build_deref_struct(b, deref, radv_ray_intersection_##field)
#define inters_load(b, deref, field)  nir_load_deref(b, inters_deref(b, deref, field))
#define inters_store(b, deref, field, value)                                                                           \
   nir_store_deref(b, inters_deref(b, deref, field), value, BITFIELD_MASK(value->num_components))
#define inters_copy(b, dst, src, field) nir_copy_deref(b, inters_deref(b, dst, field), inters_deref(b, src, field))
                    ...
                    uint32_t max_shared_size)
{
   ...
   const glsl_type *type = radv_get_ray_query_type();
   if (array_length > 1)
      type = glsl_array_type(type, array_length, 0);
...
copy_candidate_to_closest(nir_builder *b, nir_deref_instr *rq)
{
   ...
   inters_copy(b, closest, candidate, barycentrics);
   inters_copy(b, closest, candidate, geometry_id_and_flags);
   inters_copy(b, closest, candidate, instance_addr);
   inters_copy(b, closest, candidate, intersection_type);
   inters_copy(b, closest, candidate, opaque);
   inters_copy(b, closest, candidate, frontface);
   inters_copy(b, closest, candidate, sbt_offset_and_flags);
   inters_copy(b, closest, candidate, primitive_id);
   inters_copy(b, closest, candidate, t);
}
...
lower_rq_generate_intersection(nir_builder *b, nir_intrinsic_instr *instr, nir_deref_instr *rq)
{
   ...
   nir_push_if(b, nir_iand(b, nir_fge(b, inters_load(b, closest, t), instr->src[1].ssa),
                           nir_fge(b, instr->src[1].ssa, rq_load(b, rq, tmin))));
   {
      copy_candidate_to_closest(b, rq);
      inters_store(b, closest, t, instr->src[1].ssa);
   }
   nir_pop_if(b, NULL);
}
                    ...
                    struct radv_instance *instance)
{
   ...
   inters_store(b, closest, t, instr->src[7].ssa);
   inters_store(b, closest, intersection_type, nir_imm_int(b, intersection_type_none));
   ...
   inters_store(b, closest, instance_addr, accel_struct);
   inters_store(b, candidate, instance_addr, accel_struct);
...
lower_rq_load(struct radv_device *device, nir_builder *b, nir_intrinsic_instr *instr, nir_deref_instr *rq)
{
   ...
   case nir_ray_query_value_intersection_barycentrics:
      return inters_load(b, intersection, barycentrics);
   case nir_ray_query_value_intersection_candidate_aabb_opaque:
      return nir_iand(b, inters_load(b, candidate, opaque),
                      nir_ieq_imm(b, inters_load(b, candidate, intersection_type), intersection_type_aabb));
   case nir_ray_query_value_intersection_front_face:
      return inters_load(b, intersection, frontface);
   case nir_ray_query_value_intersection_geometry_index:
      return nir_iand_imm(b, inters_load(b, intersection, geometry_id_and_flags), 0xFFFFFF);
   case nir_ray_query_value_intersection_instance_custom_index: {
      nir_def *instance_node_addr = inters_load(b, intersection, instance_addr);
      return nir_iand_imm(
         b,
   ...
   case nir_ray_query_value_intersection_instance_sbt_index:
      return nir_iand_imm(b, inters_load(b, intersection, sbt_offset_and_flags), 0xFFFFFF);
   ...
   case nir_ray_query_value_intersection_primitive_index:
      return inters_load(b, intersection, primitive_id);
   case nir_ray_query_value_intersection_t:
      return inters_load(b, intersection, t);
   case nir_ray_query_value_intersection_type: {
      nir_def *intersection_type = inters_load(b, intersection, intersection_type);
      if (!committed)
   ...
   case nir_ray_query_value_intersection_triangle_vertex_positions: {
      nir_def *instance_node_addr = inters_load(b, intersection, instance_addr);
      nir_def *primitive_id = inters_load(b, intersection, primitive_id);
                      ...
                      const struct radv_ray_traversal_args *args)
{
   ...
   inters_store(b, candidate, primitive_id, intersection->primitive_id);
   inters_store(b, candidate, geometry_id_and_flags, intersection->geometry_id_and_flags);
   inters_store(b, candidate, opaque, intersection->opaque);
   inters_store(b, candidate, intersection_type, nir_imm_int(b, intersection_type_aabb));
                          ...
                          const struct radv_ray_traversal_args *args, const struct radv_ray_flags *ray_flags)
{
   ...
   inters_store(b, candidate, barycentrics, intersection->barycentrics);
   inters_store(b, candidate, primitive_id, intersection->base.primitive_id);
   inters_store(b, candidate, geometry_id_and_flags, intersection->base.geometry_id_and_flags);
   inters_store(b, candidate, t, intersection->t);
   inters_store(b, candidate, opaque, intersection->base.opaque);
   inters_store(b, candidate, frontface, intersection->frontface);
   inters_store(b, candidate, intersection_type, nir_imm_int(b, intersection_type_triangle));
                 ...
                 struct radv_device *device)
{
   ...
   struct radv_ray_traversal_vars trav_vars = {
      ...
      .instance_addr = inters_deref(b, candidate, instance_addr),
      .sbt_offset_and_flags = inters_deref(b, candidate, sbt_offset_and_flags),
   };
   ...
   nir_foreach_function (function, shader) {
      ...
      nir_foreach_block (block, function->impl) {
         nir_foreach_instr_safe (instr, block) {
            ...
            nir_deref_instr *rq = nir_build_deref_var(&builder, vars->var);
            if (ray_query_deref->deref_type == nir_deref_type_array)
               rq = nir_build_deref_array(&builder, rq, ray_query_deref->arr.index.ssa);
