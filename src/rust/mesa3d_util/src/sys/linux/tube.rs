...
impl Tube {
    pub fn new<P: AsRef<Path> + NixPath>(path: P, kind: TubeType) -> MesaResult<Tube> {
    ...
    pub fn send(&self, opaque_data: &[u8], descriptors: &[RawDescriptor]) -> MesaResult<usize> {
    ...
    pub fn receive(&self, opaque_data: &mut [u8]) -> MesaResult<(usize, Vec<OwnedDescriptor>)> {
        ...
        let descriptors = match r.cmsgs().next() {
            ...
            Some(_) => return Err(MesaError::Unsupported),
            None => Vec::new(),
        };
...
impl Listener {
    ...
    pub fn bind<P: AsRef<Path> + NixPath>(path: P) -> MesaResult<Listener> {
        let socket = socket(
            AddressFamily::Unix,
            SockType::SeqPacket,
            SockFlag::empty(),
