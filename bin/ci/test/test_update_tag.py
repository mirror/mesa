...
def test_mocked_build_script_returns_none_if_it_fails(monkeypatch, mock_build_script):
    """
    Test that run_build_script returns None if the build script fails.
    """
    mock_build_script.write_text("echo Build script which runs false\nfalse")
    tag = run_build_script("foo", check_only=False)
    assert tag is None
