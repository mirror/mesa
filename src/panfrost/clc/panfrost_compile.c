...
main(int argc, const char **argv)
{
   ...
   const char *output_h_file_name = basename(output_h_path);
   ...
   nir_foreach_entrypoint(libfunc, nir) {
      ...
      for (unsigned v = 0; v < nr_vars; ++v) {
         ...
         pan_link_cl_library(s, nir, inputs.gpu_id);
   ...
   char nir_suffix[8];
   snprintf(nir_suffix, sizeof(nir_suffix), "nir_v%d", target_arch);
   nir_precomp_print_nir(fp_c, fp_h, nir, library_name, nir_suffix);
