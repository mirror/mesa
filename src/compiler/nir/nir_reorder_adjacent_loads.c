...
{
   nir_instr *b0 = nir_get_grouped_load_binding(*a);
   nir_instr *b1 = nir_get_grouped_load_binding(*b);

   if (b0 != b1)
      return b0->index > b1->index ? 1 : -1;
...
reorder_loads(nir_instr *first, nir_instr *last, struct util_dynarray *scratch)
{
   ...
   for (i = 0, group_index = 0, instr = first; i < num_loads;
      ...
      info[i].binding = nir_get_grouped_load_binding(instr);
