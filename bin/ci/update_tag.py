...
CI_PROJECT_PATH: str | None = os.environ.get("CI_PROJECT_PATH", None)
GIT_REPO_ROOT: str = CI_PROJECT_PATH or str(Path(__file__).resolve().parent.parent.parent)
...
def run_build_script(component: str, check_only: bool = False) -> Optional[str]:
    ...
    if unbound_match:
        ...
        logging.error(f"Please set the variable {var_name}.")
        return None
    ...
    if args.check:
        ...
        for comp in args.check:
            if run_build_script(comp, check_only=True) is None:
                failed = True
        # If any component failed, exit with a non-zero exit code
        if failed:
            sys.exit(1)
        return
