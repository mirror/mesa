...
void
aco_print_physreg(PhysReg reg, FILE* output, unsigned bytes, unsigned flags)
{
   if (reg >= 128 && reg <= 192) {
...
print_definition(const Definition* definition, FILE* output, unsigned flags)
{
   ...
   if (definition->isFixed())
      aco_print_physreg(definition->physReg(), output, definition->bytes(), flags);
}
...
aco_print_operand(const Operand* operand, FILE* output, unsigned flags)
{
   if (operand->isLiteral() || (operand->isConstant() && operand->bytes() == 1)) {
      ...
      aco_print_physreg(operand->physReg(), output);
      ...
      if (operand->isFixed())
         aco_print_physreg(operand->physReg(), output, operand->bytes(), flags);
   }
}
