...
prepare_iam_sysvals(struct panvk_cmd_buffer *cmdbuf, BITSET_WORD *dirty_sysvals)
{
   ...
   for (uint32_t i = 0; i < catt_count; i++) {
      ...
      pan_pack(&iam[ia_idx].conversion, INTERNAL_CONVERSION, cfg) {
         cfg.memory_format =
...
static void
prepare_cam_sysvals(struct panvk_cmd_buffer *cmdbuf, BITSET_WORD *dirty_sysvals)
{
   uint64_t remapped_ibds[MAX_RTS] = {0};

   /* Set all targets to no_blend (AKA discard). The update for active entries
    * will happen on the GPU. */
   for (uint32_t i = 0; i < ARRAY_SIZE(remapped_ibds); i++) {
      pan_pack(&remapped_ibds[i], INTERNAL_BLEND, cfg)
         cfg.mode = MALI_BLEND_MODE_OFF;
   }

   for (uint32_t i = 0; i < ARRAY_SIZE(remapped_ibds); i++)
      set_gfx_sysval(cmdbuf, dirty_sysvals, blend.descs[i], remapped_ibds[i]);
}
                                         ...
                                         const struct panvk_draw_info *info)
{
   ...
   if (dyn_gfx_state_dirty(cmdbuf, COLOR_ATTACHMENT_MAP))
      prepare_cam_sysvals(cmdbuf, dirty_sysvals);
