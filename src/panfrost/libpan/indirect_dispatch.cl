                         ...
                         global uint32_t *num_wg_sysval_z)
{
   ...
   pan_unpack((global struct mali_job_header_packed *)indirect_job, JOB_HEADER,
              unpacked_job);
   pan_pack((global struct mali_job_header_packed *)indirect_job, JOB_HEADER,
            cfg) {
      memcpy(&cfg, &unpacked_job, sizeof(cfg));
   ...
   panfrost_pack_work_groups_compute(invocation, num_x, num_y, num_z, size_x,
                                     size_y, size_z, false, false);
#endif
   if (num_wg_sysval_x != NULL)
      *num_wg_sysval_x = num_x;

   if (num_wg_sysval_y != NULL)
      *num_wg_sysval_y = num_y;

   if (num_wg_sysval_z != NULL)
